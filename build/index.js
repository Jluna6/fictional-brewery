...
class Like {
  constructor() {
    if (document.querySelector(".like-box")) {
      (axios__WEBPACK_IMPORTED_MODULE_0___default().defaults).headers.common["X-WP-Nonce"] = breweryData.nonce;
      this.events();
    }
  }
  events() {
  ...
  async createLike(currentLikeBox) {
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default().post(breweryData.root_url + "/wp-json/brewery/v1/manageLike", {
  ...
  async deleteLike(currentLikeBox) {
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default()({
        url: breweryData.root_url + "/wp-json/brewery/v1/manageLike",
        method: "delete",
        data: {
...
class MyNotes {
  ...
  async deleteNote(e) {
    const thisNote = this.findNearestParentLi(e.target);
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default()["delete"](breweryData.root_url + "/wp-json/wp/v2/note/" + thisNote.getAttribute("data-id"));
  ...
  async updateNote(e) {
    ...
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default().post(breweryData.root_url + "/wp-json/wp/v2/note/" + thisNote.getAttribute("data-id"), ourUpdatedPost);
      this.makeNoteReadOnly(thisNote);
    } catch (e) {
      console.log("Sorry");
    }
  ...
  async createNote() {
    ...
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default().post(breweryData.root_url + "/wp-json/wp/v2/note/", ourNewPost);
...
class Search {
  ...
  async getResults() {
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default().get(breweryData.root_url + "/wp-json/brewery/v1/search?term=" + this.searchField.value);
      ...
      this.resultsDiv.innerHTML = `
        <div class="row">
          ...
          <div class="one-third">
            ...
            ${results.beer_types.length ? '<ul class="link-list min-list">' : `<p>No beer types match that search. <a href="${breweryData.root_url}/beer_types">View all beer types</a></p>`}
              ${results.beer_types.map(item => `<li><a href="${item.permalink}">${item.title}</a></li>`).join("")}
            ${results.beer_types.length ? "</ul>" : ""}

            <h2 class="search-overlay__section-title">Beers</h2>
            ${results.beers.length ? '<ul class="beer-cards">' : `<p>No beers match that search.</p>`}
              ${results.beers.map(item => `
          ...
          <div class="one-third">
            ...
            ${results.brew_pubs.length ? '<ul class="link-list min-list">' : `<p>No  brew pubs match that search. <a href="${breweryData.root_url}/brew_pubs">View all brew pubs</a></p>`}
              ${results.brew_pubs.map(item => `<li><a href="${item.permalink}">${item.title}</a></li>`).join("")}
            ${results.brew_pubs.length ? "</ul>" : ""}

            <h2 class="search-overlay__section-title">Events</h2>
            ${results.events.length ? "" : `<p>No events match that search. <a href="${breweryData.root_url}/events">View all events</a></p>`}
...
/******/ 		}
/******/ 		
/******/ 		var chunkLoadingGlobal = globalThis["webpackChunkfictional_brewery_theme"] = globalThis["webpackChunkfictional_brewery_theme"] || [];
